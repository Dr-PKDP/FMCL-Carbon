...
simulation.py
=============
Core federated-learning simulation engine for:
...
What this file does
-------------------
Defines the three building blocks used by floor_scaling.py and
make_figures.py:

  make_federated_data()  –  synthetic non-IID dataset (Dirichlet partition)
  run()                  –  one FedProx training campaign with optional
                            Bernoulli dropout, DP noise, and correlated
                            participation

This file is NOT meant to be run directly.  Run make_figures.py instead.

Dependencies
------------
    numpy, scipy
    (matplotlib is only needed in make_figures.py)

Reproducibility
---------------
All randomness is seeded through numpy's Generator interface.
Pass a different seed= to run() to get independent replications.
"""
